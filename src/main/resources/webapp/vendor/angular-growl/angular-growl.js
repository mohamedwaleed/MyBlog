  ...
  this.$get = [
    ...
    function ($rootScope, $interpolate, $sce, $filter, $timeout, growlMessages) {
      ...
      function sendMessage(text, config, severity) {
		...
		$rootScope.notificationMessages.push(message);
		//alert('sassasa');
...
angular.module('angular-growl').service('growlMessages', [
  ...
  '$timeout',
  '$rootScope',
  function ($sce, $timeout,$rootScope) {
    'use strict';
    this.directives = {};
    ...
    this.deleteMessages = function () {
          for(var index=0;index<$rootScope.notificationMessages.length;index++){
            this.deleteMessage($rootScope.notificationMessages[index]);
          }
          $rootScope.notificationMessages=[];
     };

     this.deleteErrorMessages = function(){
        for(var index=0;index<$rootScope.notificationMessages.length;index++){
          if($rootScope.notificationMessages[index].severity == 'error')
            this.deleteMessage($rootScope.notificationMessages[index]);
        }
